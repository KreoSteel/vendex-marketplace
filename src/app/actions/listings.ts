...
type ActionState = { error: string; success?: string } | { success: string; error?: string } | null;
...
export async function createListingAction(
   ...
   const { urls, error: uploadError } = await uploadImages(files, user.id);
   ...
   const parsedListing = createListingSchema.safeParse(data);
   if (!parsedListing.success) {
      return { error: parsedListing.error.message };
   }

   try {
      await createListing(parsedListing.data);
