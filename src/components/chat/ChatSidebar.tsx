...
export default function ChatSidebar() {
    ...
    const { mutate: changeMessageReadStatus } = useChangeMessageReadStatus();
    ...
    useEffect(() => {
      ...
      const activeConversations = conversations.find((conversation) => pathname === `/messages/${conversation.otherUser.id}`);

      if (activeConversations && !activeConversations.read && !markedAsReadRef.current.has(activeConversations.otherUser.id)) {
      ...
      <div className="flex flex-col h-full w-full md:w-80 lg:w-96 border-r bg-white">
        ...
        <div className="flex-1 overflow-y-auto">
           {conversations?.map((conversation) => {
            const { otherUser, lastMessage, lastMessageAt, read } = conversation;
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive && "bg-blue-50 hover:bg-blue-50 border-l-4 border-l-blue-500"
                  )}
                >
